...
void signalEfficiency(){
  ...
  for( int i = 0; i < N; ++i ){
    ...
    y_eff_elc1[i] = getEleEfficiency(Form("/data7/htong/skim_samples/ele/skim_ele_ZprimeToZhToZlephbb_narrow_M-%d_13TeV-madgraph.root",(Int_t)x_mzh[i]),1);
    y_eff_elc2[i] = getEleEfficiency(Form("/data7/htong/skim_samples/ele/skim_ele_ZprimeToZhToZlephbb_narrow_M-%d_13TeV-madgraph.root",(Int_t)x_mzh[i]),2);
    y_eff_muc1[i] = getEleEfficiency(Form("/data7/htong/skim_samples/mu/skim_mu_ZprimeToZhToZlephbb_narrow_M-%d_13TeV-madgraph.root",(Int_t)x_mzh[i]),1);
    y_eff_muc2[i] = getEleEfficiency(Form("/data7/htong/skim_samples/mu/skim_mu_ZprimeToZhToZlephbb_narrow_M-%d_13TeV-madgraph.root",(Int_t)x_mzh[i]),2);

  }

  TGraph *g_eff_elc1 = new TGraph(N, x_mzh, y_eff_elc1);
  TGraph *g_eff_elc2 = new TGraph(N, x_mzh, y_eff_elc2);
  TGraph *g_eff_muc1 = new TGraph(N, x_mzh, y_eff_muc1);
  TGraph *g_eff_muc2 = new TGraph(N, x_mzh, y_eff_muc2);
