...
for ((i=0; i<${#ch[@]}; i++)); do
    ...
    root -q -b -l pdfScaleTree.C\(\"$samplePath/skim_${ch[$i]}_crab_WW_TuneCUETP8M1_13TeV-pythia8.root\"\,\"WW_TuneCUETP8M1_13TeV\"\,\"${ch[$i]}\"\)
    root -q -b -l pdfScaleTree.C\(\"$samplePath/skim_${ch[$i]}_crab_WZ_TuneCUETP8M1_13TeV-pythia8.root\"\,\"WZ_TuneCUETP8M1_13TeV\"\,\"${ch[$i]}\"\)
    root -q -b -l pdfScaleTree.C\(\"$samplePath/skim_${ch[$i]}_crab_ZZ_TuneCUETP8M1_13TeV-pythia8.root\"\,\"ZZ_TuneCUETP8M1_13TeV\"\,\"${ch[$i]}\"\)

    echo "Processing ZH background..."

    root -q -b -l pdfScaleTree.C\(\"$samplePath/skim_${ch[$i]}_crab_ZH_HToBB_ZToLL_M125_13TeV_powheg_pythia8.root\"\,\"ZH_HToBB_ZToLL_M125_13TeV\"\,\"${ch[$i]}\"\)

    echo "Processing ttbar background..."

    root -q -b -l pdfScaleTree.C\(\"$samplePath/skim_${ch[$i]}_crab_TT_TuneCUETP8M1_13TeV-powheg-pythia8.root\"\,\"TT_TuneCUETP8M1_13TeV\"\,\"${ch[$i]}\"\)
