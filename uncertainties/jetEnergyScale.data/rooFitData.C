...
void rooFitData(string channel, string catcut, bool removeMinor=true){
  ...
  for(int nw = 2; nw >= 0; --nw){
    ...
    if( channel == "ele" ){
      ...
      treeData->Add(Form("%s/data/SingleElectron-Run2015D-v1_%s_toyMC.root", channel.data(), region[nw].data()));
      treeData->Add(Form("%s/data/SingleElectron-Run2015D-v4_%s_toyMC.root", channel.data(), region[nw].data()));

    }

    else if( channel == "mu" ){

      treeData->Add(Form("%s/data/SingleMuon-Run2015D-v1_%s_toyMC.root", channel.data(), region[nw].data()));
      treeData->Add(Form("%s/data/SingleMuon-Run2015D-v4_%s_toyMC.root", channel.data(), region[nw].data()));
    ...
    if( removeMinor ){
      ...
      treeData->Add(Form("%s/VV/WW_TuneCUETP8M1_13TeV_%s_toyMC.root",     channel.data(), region[nw].data()));
      treeData->Add(Form("%s/VV/WZ_TuneCUETP8M1_13TeV_%s_toyMC.root",     channel.data(), region[nw].data()));
      treeData->Add(Form("%s/VV/ZZ_TuneCUETP8M1_13TeV_%s_toyMC.root",     channel.data(), region[nw].data()));
      treeData->Add(Form("%s/TT/TT_TuneCUETP8M1_13TeV_%s_toyMC.root",     channel.data(), region[nw].data()));
      treeData->Add(Form("%s/ZH/ZH_HToBB_ZToLL_M125_13TeV_%s_toyMC.root", channel.data(), region[nw].data()));
