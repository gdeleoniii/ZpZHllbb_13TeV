...
void mZHmuLimitV2(std::string inputFile, std::string outputFile){
  ...
  TH1D* h_mZprime_tight = new TH1D("h_mZprime_tight", "mZprime_tight", 100, 400, 5000);
  TH1D* h_mZprime_loose = new TH1D("h_mZprime_loose", "mZprime_loose", 100, 400, 5000);
  TH1D* h_eventWeight   = new TH1D("h_eventWeight",   "eventWeight",     2,  -1,    1);

  h_mZprime_tight->Sumw2();
  h_mZprime_loose->Sumw2();

  h_mZprime_tight->GetXaxis()->SetTitle("mZprime");
  h_mZprime_loose->GetXaxis()->SetTitle("mZprime");
  ...
  for( Long64_t ev = 0; ev < data.GetEntriesFast(); ev++){
	...
	if( FATsubjetSDCSV[ij][is] > 0.605 ) nsubBjet++;
      ...
      if( ev % 2 == 0 && nsubBjet < 2 ) continue;
      else if(ev % 2 == 1 && nsubBjet < 1 ) continue;
    ...
    if( ev % 2 == 0 )
      h_mZprime_tight->Fill(mllbb,eventWeight);

    else if( ev % 2 == 1 )
      h_mZprime_loose->Fill(mllbb,eventWeight);

  } // end of event loop

  fprintf(stderr, "Processed all events\n");

  TFile* outFile = new TFile(Form("%s_mZHmuSignalV2.root",outputFile.c_str()), "recreate");

  h_mZprime_tight->Write("mZprime_tight");
  h_mZprime_loose->Write("mZprime_loose");
