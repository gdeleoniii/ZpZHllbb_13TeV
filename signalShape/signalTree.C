...
void signalTree(string inputFile, string outputFile, string channel, int mzh){
  ...
  float scale = 2512.*readHist::crossSection(inputFile.data())/((TH1F*)f.Get("h_totalEv"))->Integral();
  ...
  for( Long64_t ev = data.GetEntriesFast()-1; ev >= 0; --ev ){
    ...
    evweight = eventWeight * scale * btagWeight * thisLepWeight * thatLepWeight * muTrigWeight;
