...
void muHighPtVariable(std::string inputFile, std::string outputFile){
  ...
  for( Long64_t ev = data.GetEntriesFast()-1; ev >= 0; --ev ){
    ...
    for(Int_t im = 0; im < 2; ++im){
      ...
      if( isGlobalMuon[muId[im]] ){ // highptMuon selections and cuts

	for(Int_t flag = 0; flag <= 7; ++flag){

	  if( muHits[muId[im]]      <  1   && flag != 0 ) continue;
	  if( muMatches[muId[im]]   <  2   && flag != 1 ) continue;
	  if( muTrkLayers[muId[im]] <  6   && flag != 2 ) continue;
	  if( muPixelHits[muId[im]] <  1   && flag != 3 ) continue;
	  if( muTrkPtErr[muId[im]]/muTrkPt[muId[im]] > 0.3 && flag != 4 ) continue;
	  if( fabs(mudxy[muId[im]]) >  0.2 && flag != 5 ) continue;
	  if( fabs(mudz[muId[im]])  >  0.5 && flag != 6 ) continue;
	  if( muMiniIsoEA[muId[im]] >  0.2 && flag != 7 ) continue;
	      
	  switch(flag){

	  case 0: h_muHits      ->Fill(muHits[muId[im]],eventWeight);      break;
	  case 1: h_muMatches   ->Fill(muMatches[muId[im]],eventWeight);   break;
	  case 2: h_muTrkLayers ->Fill(muTrkLayers[muId[im]],eventWeight); break;
	  case 3: h_muPixelHits ->Fill(muPixelHits[muId[im]],eventWeight); break;
	  case 4: h_muTrkPtErrdvTrkPt ->Fill(muTrkPtErr[muId[im]]/muTrkPt[muId[im]],eventWeight); break;
	  case 5: h_mudxy       ->Fill(mudxy[muId[im]],eventWeight);       break; 
	  case 6: h_mudz        ->Fill(mudz[muId[im]],eventWeight);        break;
	  case 7: h_muMiniIsoEA ->Fill(muMiniIsoEA[muId[im]],eventWeight); break;
